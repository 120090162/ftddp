...
class DAM_contact(crocoddyl.DifferentialActionModelAbstract):
    ...
    def collision_test(self,data,tau,v):
        ...
        h = []
        if clamping_ids != []:
            Jc = np.concat([J[(3*i):(3*i+3),:] for i in clamping_ids],axis=0)
            impulsec = np.concat([impulse[(3*i):(3*i+3)] for i in clamping_ids])
            Jleft.append(Jc)
            Jright.append(Jc)
            J_.append(Jc)
            contact_impulse.append(impulsec)
            h.append(np.concat( [np.array([0,0,height[i]]) for i in clamping_ids] ))
        ...
        if slide_ids != []:
            Es = block_diag(*Es)
            data.Es = Es
            Jsn = np.concat([J[(3*i+2):(3*i+3),:] for i in slide_ids],axis=0)
            Jst = np.concat([J[(3*i):(3*i+2),:] for i in slide_ids],axis=0)
            Js = np.concat([J[(3*i):(3*i+3),:] for i in slide_ids],axis=0)
            impulsesn = np.array([impulse[(3*i+2)] for i in slide_ids])
            impulsest = np.concat([impulse[(3*i):(3*i+2)] for i in slide_ids])
            Jleft.append(Jsn)
            ...
            h.append(np.concat( [np.array([height[i]]) for i in slide_ids] ))
        ...
        Jleft = np.concat(Jleft, axis=0)        
        Jright = np.concat(Jright, axis=0)
        ...
        data.contactJ = np.concat(J_,axis=0)
    ...
    def calcDiff(self, data, x, u=None):
        if u is None:
            ...
            #     data.Minv, data.actuation.dtau_dx
            # )
            data.Fx[:, :] = np.concat([ddq_dq, ddq_dv],axis=1) + data.Minv@data.actuation.dtau_dx
            ...
            def preFq_symfun(q_sym):
                ...
                sxpreb = SX(data.contactpreb)
                sxAinv = SX(contactAinv)
                sximpulse = SX(impulse)
                ...
                for fid in clamping_ids+slide_ids:
                    ...
                    if fid in clamping_ids:
                        ...
                        Jst.append(Ji[:2,:])
                        Jsright.append(
                            SX(Es[(3*k):(3*k+3),k:(k+1)].T) @ Ji
                        )
                        h.append(hi)
                J = vertcat(*(Jc+Jsn+Jst))
                Jleft = vertcat(*(Jc+Jsn))
                Jright = vertcat(*(Jc+Jsright))
                
                h = vertcat(*h)
                A = Jleft@cdata.Minv@Jright.T
                
                if slide_ids == []:
                    lambda_pre = -sxAinv@A@sximpulse
                else:
                    lambda_pre = -sxAinv@A@sximpulse[:-(2*len(slide_ids))]
                b_pre = Jleft@sxpreb                
                Fq_pre = J.T@sximpulse
                return lambda_pre, b_pre,Fq_pre, h
            lambda_presym, b_presym,Fq_presym, hsym = preFq_symfun(qsym)
            dlambda_presym = jacobian(lambda_presym,qsym)
            db_presym = jacobian(b_presym,qsym)
            dFq_presym = jacobian(Fq_presym,qsym)
            dh_dqsym = jacobian(hsym,qsym)
            dlambda_prefun = Function('jacobian', [qsym], [dlambda_presym])
            db_prefun = Function('jacobian', [qsym], [db_presym])
            dFq_prefun = Function('jacobian', [qsym], [dFq_presym])
            dh_dqfun = Function('jacobian', [qsym], [dh_dqsym])
            dlambda_pre = np.array(dlambda_prefun(qrpy))
            db_pre = np.array(db_prefun(qrpy))
            dFq_pre = np.array(dFq_prefun(qrpy))
            dh_dq = np.array(dh_dqfun(qrpy))
            ...
            #         return cdata.oMf[collision_ids[fid]].translation[2]
            ...
            db_dq = db_pre + contactJleft@ddq_dq*self.dt
            ...
            dlambda_dq = dlambda_pre - contactAinv@(db_dq+dh_dq/self.dt)
            ...
            if slide_ids != []:
                ...
                dlambda_dq = np.concat([dlambda_dq, Est@dlambda_dq[-snum:,:]],axis=0)
                dlambda_dv = np.concat([dlambda_dv, Est@dlambda_dv[-snum:,:]],axis=0)
                dlambda_dtau = np.concat([dlambda_dtau, Est@dlambda_dtau[-snum:,:]],axis=0)
            
            # Fq = Minv@ ( (dJ_dq.transpose(0,2,1)@(impulse[np.newaxis,:,np.newaxis])).squeeze().T + contactJ.T@dlambda_dq )/self.dt
            Fq = Minv@ (dFq_pre + contactJ.T@dlambda_dq) /self.dt
            Fv = Minv@contactJ.T@dlambda_dv/self.dt
            Ftau = Minv@contactJ.T@dlambda_dtau/self.dt
            
            data.Fx[:, :] += np.concat([Fq, Fv],axis=1)
